...
class GitRunContext(RunContextProvider):
    ...
    def _git_branch(self):
        if "git_branch" not in self._cache:
            main_file = _get_main_file()
            self._cache["git_branch"] = get_git_branch(main_file) if main_file is not None else None
    ...
    def _git_repo_url(self):
        if "git_repo_url" not in self._cache:
            main_file = _get_main_file()
            self._cache["git_repo_url"] = get_git_repo_url(main_file) if main_file is not None else None
        return self._cache["git_repo_url"]

    def in_context(self):
        return (
            self._source_version is not None
            or self._git_branch is not None
            or self._git_repo_url is not None
        )
