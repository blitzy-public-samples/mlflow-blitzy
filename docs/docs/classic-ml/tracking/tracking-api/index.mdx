...
| `mlflow.source.git.branch`  | Git branch name                          | When run from git repo |
| `mlflow.source.git.repoURL` | Git repository URL                       | When run from git repo |
...
MLflow automatically captures Git metadata when runs are started from within a Git repository, requiring no changes to your existing code.

The following system tags are automatically set when available:

- `mlflow.source.git.commit` - The current Git commit hash
- `mlflow.source.git.branch` - The current Git branch name
- `mlflow.source.git.repoURL` - The Git repository URL (typically from the first remote)

These tags are set automatically without requiring any code changes when you call `mlflow.start_run()` or create runs through the tracking client.

:::note
When Git information is unavailable (such as detached HEAD state or running outside a Git repository), these tags are simply not set and no errors are raised. Your runs will continue to work normally.
:::

If you manually set any of these Git tags using `mlflow.set_tag()`, your manually set values will take precedence over automatically detected values.
