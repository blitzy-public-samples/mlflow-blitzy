...
:::info Automatic Git Metadata Tracking
When you start an MLflow run from within a Git repository, MLflow automatically captures the current Git commit hash, branch name, and repository URL as system tags. This happens transparently without requiring any code changes, making it easy to trace experiment results back to the exact code version that produced them. These tags are set when Git information is available; if any information cannot be detected (e.g., detached HEAD state), that particular tag is simply omitted.
:::
