...
**Based on the prompt, the Blitzy platform understands that the new feature requirement is to automatically track Git metadata (branch name and repository URL) for all MLflow experiments, regardless of whether they're using MLflow Projects or MLflow Recipe.**

The platform has identified the following explicit and implicit requirements:
- **Explicit Requirements**:
  - Automatically detect and log Git branch name as `mlflow.source.git.branch` system tag
  - Automatically detect and log Git repository URL as `mlflow.source.git.repoURL` system tag
  - Feature must work transparently when `mlflow.start_run()` is called within a Git repository
  - No user code changes required - completely automatic detection
  - Must not interfere with existing MLflow Projects or Recipe Git tracking
  - Preserve backward compatibility - existing code must continue to work

- **Implicit Requirements Detected**:
  - Must handle edge cases gracefully (detached HEAD, no remotes, permission issues)
  - Should not significantly impact run creation performance
  - Must work across different operating systems (Windows, Linux, macOS)
  - Should leverage existing Git utility functions to avoid code duplication
  - Must maintain thread safety for concurrent run creation scenarios
  - Should cache Git information for performance optimization
  - Must not override manually set Git tags

#### Special Instructions and Constraints

**CRITICAL: The Blitzy platform has captured the following specific directives:**

- **Integration Constraint**: "Only modify files within the tracking client module (`mlflow/tracking/`) and related utilities"
- **Architectural Requirement**: "Keep changes isolated to the run creation/initialization flow"
- **Preservation Requirement**: "Do not modify the MLflow Projects or Recipe components"
- **Compatibility Constraint**: "All existing MLflow Projects functionality and its Git tracking behavior must remain untouched"
- **Interface Stability**: "The public API signatures of `mlflow.start_run()`, `mlflow.end_run()`, and `MlflowClient` methods must remain unchanged"
- **Minimal Change Discipline**: "Make only the changes that are absolutely necessary to implement this feature"

**User Examples Preserved**:
- User Example: "A data scientist runs `mlflow.start_run()` in a Jupyter notebook or Python script within a Git repository, and MLflow automatically captures the current branch and repository URL without any additional code or configuration"
- User Example: "A team running distributed experiments can trace any experiment result back to the exact Git branch and repository it came from, enabling better collaboration and debugging"

#### Technical Interpretation

**These feature requirements translate to the following technical implementation strategy:**

1. **To automatically detect Git metadata**, we will enhance the existing `GitRunContext` provider in `mlflow/tracking/context/git_context.py` to call additional Git utility functions

2. **To capture branch name**, we will invoke the existing `get_git_branch()` function from `mlflow/utils/git_utils.py` and add it to the tags dictionary

3. **To capture repository URL**, we will invoke the existing `get_git_repo_url()` function from `mlflow/utils/git_utils.py` and add it to the tags dictionary

4. **To ensure transparency**, we will leverage the existing RunContextProviderRegistry which automatically applies context providers during run creation

5. **To maintain backward compatibility**, we will not modify any public API signatures and will gracefully handle cases where Git information is unavailable

6. **To avoid conflicts with MLflow Projects**, we will ensure our automatic detection does not interfere with the manual Git tag setting in `mlflow/projects/utils.py`

7. **To optimize performance**, we will extend the existing caching mechanism in `GitRunContext` to cache branch and repo URL alongside the commit hash

## 0.2 Repository Scope Discovery

#### Comprehensive File Analysis

**Based on the Blitzy platform's deep analysis of the MLflow repository, the following files have been identified as requiring modification or creation:**

| File Path | Action | Purpose |
|-----------|--------|---------|
| `mlflow/tracking/context/git_context.py` | MODIFY | Add branch and repo URL capture to GitRunContext provider |
| `mlflow/utils/git_utils.py` | NO CHANGE | Already contains necessary functions: `get_git_branch()` and `get_git_repo_url()` |
| `mlflow/utils/mlflow_tags.py` | NO CHANGE | Already defines required constants: `MLFLOW_GIT_BRANCH` and `MLFLOW_GIT_REPO_URL` |
| `tests/tracking/context/test_git_context.py` | MODIFY | Add tests for new branch and repo URL tagging behavior |
| `tests/tracking/fluent/test_fluent_git.py` | CREATE | New integration tests for Git metadata in fluent API |

**Integration Point Discovery:**

The platform has identified the following critical integration points that will be affected:

1. **Run Context Provider Registry** (`mlflow/tracking/context/registry.py`)
   - Already registers `GitRunContext` during module initialization
   - No modification needed - will automatically pick up enhanced GitRunContext behavior

2. **Fluent API Entry Points** (`mlflow/tracking/fluent.py`)
   - `start_run()` function at lines ~300-500 calls `resolve_tags()` from registry
   - No modification needed - uses existing context provider infrastructure

3. **Tracking Client** (`mlflow/tracking/client.py`)
   - `create_run()` method uses context providers via registry
   - No modification needed - delegates to context resolution

4. **MLflow Projects Module** (`mlflow/projects/utils.py`)
   - Lines ~600-700: Contains separate manual Git tag setting in `_create_run()`
   - **CRITICAL: Must NOT be modified** - preserves existing MLflow Projects behavior

#### Web Search Research Conducted

<cite index="1-14,2-19,2-20">Based on research of Git metadata retrieval best practices, the platform has identified that GitPython's `repo.active_branch.name` provides the current branch name and `repo.remotes.origin.url` provides the repository URL.</cite> <cite index="5-17,5-18">The recommended approach uses `git symbolic-ref --short HEAD` for branch detection and handles edge cases gracefully.</cite>

**Key Research Findings:**
- <cite index="7-19">GitPython provides high-level access to repository data through `Repo` objects</cite>
- <cite index="3-9,3-12">Repository URL extraction requires parsing `repo.remotes.origin.url` and handling multiple remotes</cite>
- <cite index="4-13">Commands like `git rev-parse --abbrev-ref HEAD` are ideal for scripting</cite>
- Edge cases include detached HEAD state, repositories without remotes, and submodules
...
| File Path | Purpose |
|-----------|---------|
| `tests/tracking/fluent/test_fluent_git.py` | Integration tests validating Git metadata capture through fluent API |

**Content Structure for New Test File:**
```python
# Test cases to implement:
# - test_start_run_captures_git_metadata()
# - test_start_run_without_git_repo()
# - test_start_run_detached_head()
# - test_start_run_no_remotes()
# - test_nested_runs_inherit_git_metadata()
# - test_git_metadata_not_overwritten()
```

#### Files Explicitly OUT OF SCOPE

**The following files must NOT be modified to preserve existing functionality:**

| File Path | Reason for Exclusion |
|-----------|---------------------|
| `mlflow/projects/utils.py` | Contains separate MLflow Projects Git tracking - must remain independent |
| `mlflow/projects/*.py` | All MLflow Projects files - preserve existing Project Git behavior |
| `mlflow/recipes/*.py` | MLflow Recipe components - maintain current Recipe functionality |
| `mlflow/server/*.py` | Server components - no server-side changes required |
| `mlflow/store/db/*.py` | Database schema - no schema modifications needed |
| `mlflow/entities/*.py` | Entity definitions - no changes to data models |
| `mlflow/protos/*.proto` | Protocol buffers - no API contract changes |

#### Repository Structure Analysis

**The Blitzy platform's analysis reveals the following repository organization:**

```
mlflow/
├── tracking/              # Primary modification area
│   ├── context/          # Git context provider location
│   │   ├── git_context.py    # MODIFY: Add branch/URL capture
│   │   └── registry.py       # NO CHANGE: Already registers providers
│   └── fluent.py             # NO CHANGE: Uses context registry
├── utils/                    # Utilities already in place
│   ├── git_utils.py         # NO CHANGE: Has required functions
│   └── mlflow_tags.py       # NO CHANGE: Has required constants
├── projects/                 # DO NOT MODIFY
│   └── utils.py             # Has separate Git tracking
└── tests/
    └── tracking/
        └── context/
            └── test_git_context.py  # MODIFY: Add new test cases
```

#### Validation of Existing Infrastructure

**The platform confirms the following existing infrastructure is sufficient:**

1. **Git Utility Functions** - Already implemented in `mlflow/utils/git_utils.py`:
   - `get_git_branch(path=None)` - Returns current branch name
   - `get_git_repo_url(path=None)` - Returns repository URL
   - `get_git_commit(path=None)` - Already used by GitRunContext

2. **Tag Constants** - Already defined in `mlflow/utils/mlflow_tags.py`:
   - `MLFLOW_GIT_BRANCH = "mlflow.source.git.branch"`
   - `MLFLOW_GIT_REPO_URL = "mlflow.source.git.repoURL"`

3. **Context Provider Infrastructure** - Fully operational:
   - `RunContextProvider` abstract base class
   - `RunContextProviderRegistry` for automatic discovery
   - `resolve_tags()` function for tag aggregation

## 0.3 Integration Analysis
...
```python
# mlflow/tracking/context/git_context.py - Current implementation
class GitRunContext(RunContextProvider):
    def tags(self):
        return {MLFLOW_GIT_COMMIT: self._source_version}  # Line ~45
```

**Modification needed at line ~45:**
- Extend the tags dictionary to include branch and repo URL
- Import additional constants from `mlflow.utils.mlflow_tags`
- Import additional functions from `mlflow.utils.git_utils`

```python
# After modification - enhanced tags method
def tags(self):
    tags = {MLFLOW_GIT_COMMIT: self._source_version}
    
    # Add new Git metadata
    if self._git_branch:
...
```python
# tests/tracking/context/test_git_context.py - Line ~50
def test_git_run_context_tags(patch_script_name, patch_git_repo):
    # Current test only checks MLFLOW_GIT_COMMIT
    # Must be extended to verify MLFLOW_GIT_BRANCH and MLFLOW_GIT_REPO_URL
```

#### Dependency Injection Points

**No dependency injection required** - The existing infrastructure already handles all necessary wiring:

1. **Context Provider Registration** (`mlflow/tracking/context/registry.py`):
   ```python
   # Line ~30 - GitRunContext already registered
   _run_context_provider_registry.register(GitRunContext)
   ```

2. **Tag Resolution** (`mlflow/tracking/fluent.py`):
   ```python
   # Line ~400 - Already calls context registry
   tags = resolve_tags(tags)  # Automatically includes GitRunContext
   ```
...
- Git metadata stored as regular run tags in existing tag tables
- No new columns, tables, or migrations needed
- Tags table already supports arbitrary key-value pairs
- System tags with `mlflow.` prefix already supported

#### Integration Flow Diagram

```mermaid
graph TD
    A[User calls mlflow.start_run] --> B[fluent.py: start_run function]
    B --> C[registry.py: resolve_tags]
    C --> D[Iterate through registered providers]
    D --> E[GitRunContext.in_context check]
    E -->|True| F[GitRunContext.tags called]
    F --> G[Get commit via get_git_commit]
    F --> H[Get branch via get_git_branch]
    F --> I[Get repo URL via get_git_repo_url]
    G --> J[Return tags dictionary]
    H --> J
    I --> J
    J --> K[Merge tags into run metadata]
    K --> L[Create/update run with tags]
    
    M[MLflow Projects] --> N[projects/utils.py: _create_run]
    N --> O[Manual Git detection]
    O --> P[Set Git tags directly]
...
#### Critical Non-Interference Points

**The following integration points must remain untouched to preserve existing behavior:**

1. **MLflow Projects Git Tracking** (`mlflow/projects/utils.py`):
   ```python
   # Lines 600-650 - DO NOT MODIFY
   def _create_run(uri, experiment_id, work_dir, entry_point):
       # ... existing code ...
       tags[MLFLOW_GIT_COMMIT] = get_git_commit(work_dir)
       tags[MLFLOW_GIT_BRANCH] = get_git_branch(work_dir)  
       tags[MLFLOW_GIT_REPO_URL] = get_git_repo_url(work_dir)
       # This manual implementation must remain separate
   ```

2. **MLflow Recipe Git Tracking**:
   - No Recipe-specific Git tracking found in codebase
   - Recipe uses standard tracking APIs which will benefit from enhancement

#### API Compatibility Matrix

| API Method | Current Behavior | After Enhancement | Breaking Change? |
|------------|------------------|-------------------|------------------|
| `mlflow.start_run()` | May set `mlflow.source.git.commit` | Also sets branch and repo URL | No |
| `mlflow.end_run()` | No Git behavior | No change | No |
| `MlflowClient.create_run()` | Uses context providers | Inherits enhancement | No |
| `mlflow.projects.run()` | Manual Git tagging | Unchanged | No |
| Manual tag setting | User can set Git tags | Not overwritten | No |

#### Thread Safety Analysis

**The current implementation has a known thread safety consideration:**
...
**Mitigation Strategy:**
- The caching mechanism is per-instance and instances are typically short-lived
- Run creation is typically not highly concurrent within a single process
- If needed, could use `threading.Lock` but not required for initial implementation

#### Performance Impact Assessment

**Expected Performance Characteristics:**

| Operation | Current Time | Expected Time | Impact |
|-----------|--------------|---------------|---------|
| `get_git_commit()` | ~10-50ms | No change | Already cached |
| `get_git_branch()` | N/A | ~5-10ms | Minimal - cached |
| `get_git_repo_url()` | N/A | ~5-10ms | Minimal - cached |
| **Total overhead** | ~10-50ms | ~20-70ms | Acceptable |

**Caching Strategy:**
- GitRunContext already implements caching for commit
- Extend caching to include branch and repo URL
- Cache populated once per GitRunContext instance
- Prevents repeated Git operations

## 0.4 Technical Implementation

#### File-by-File Execution Plan

**CRITICAL: Every file listed here MUST be modified or created as specified**
...
class GitRunContext(RunContextProvider):
    ...
    @property
    def _source_version(self) -> Optional[str]:
        # Existing implementation unchanged
        return self._cache.get("source_version")
    ...
    def _git_branch(self) -> Optional[str]:
        if "git_branch" not in self._cache:
            ...
            self._cache["git_branch"] = get_git_branch(main_file) if main_file else None
        return self._cache.get("git_branch")
    
    @property
    def _git_repo_url(self) -> Optional[str]:
        if "git_repo_url" not in self._cache:
            main_file = _get_main_file()
            self._cache["git_repo_url"] = get_git_repo_url(main_file) if main_file else None
        return self._cache.get("git_repo_url")
    
    def in_context(self) -> bool:
        # MODIFY: Check if any Git metadata is available
        return self._source_version is not None or \
               self._git_branch is not None or \
               self._git_repo_url is not None
    
    def tags(self) -> dict:
        # MODIFY: Return all available Git metadata
        tags = {}
...
```python
# Add to imports section (line ~10)
from mlflow.utils.mlflow_tags import MLFLOW_GIT_BRANCH, MLFLOW_GIT_REPO_URL

#### Add new test constants (line ~15)
MOCK_BRANCH_NAME = "feature/test-branch"
MOCK_REPO_URL = "https://github.com/mlflow/mlflow.git"

#### Modify patch_git_repo fixture (line ~30)
@pytest.fixture
def patch_git_repo():
    mock_repo = mock.Mock()
    mock_repo.head.commit.hexsha = MOCK_COMMIT_HASH
    mock_repo.active_branch.name = MOCK_BRANCH_NAME  # ADD THIS
    mock_repo.remotes.origin.url = MOCK_REPO_URL    # ADD THIS
    mock_repo.ignored.return_value = []
    with mock.patch("git.Repo", return_value=mock_repo) as mock_repo_class:
        yield mock_repo_class

#### Modify test_git_run_context_tags (line ~50)
def test_git_run_context_tags(patch_script_name, patch_git_repo):
    assert GitRunContext().tags() == {
        MLFLOW_GIT_COMMIT: MOCK_COMMIT_HASH,
        MLFLOW_GIT_BRANCH: MOCK_BRANCH_NAME,     # ADD THIS
        MLFLOW_GIT_REPO_URL: MOCK_REPO_URL       # ADD THIS
    }
...
def test_git_run_context_partial_metadata(patch_script_name):
    ...
    with mock.patch("git.Repo", return_value=mock_repo):
        ...
        assert MLFLOW_GIT_REPO_URL not in tags  # No remote URL
...
def test_git_run_context_detached_head(patch_script_name):
    ...
    mock_repo.active_branch = None  # Detached HEAD
    mock_repo.remotes.origin.url = MOCK_REPO_URL
    mock_repo.ignored.return_value = []
    ...
    with mock.patch("git.Repo", return_value=mock_repo):
        with mock.patch("mlflow.utils.git_utils.get_git_branch", return_value=None):
            ...
            tags = context.tags()
            assert MLFLOW_GIT_COMMIT in tags
            assert MLFLOW_GIT_BRANCH not in tags  # No branch in detached state
            assert MLFLOW_GIT_REPO_URL in tags
```

**CREATE: `tests/tracking/fluent/test_fluent_git.py`**
```python
...
def test_start_run_captures_git_metadata():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Mock the main file to be in our test repo
        ...
        with mock.patch("mlflow.tracking.context.default_context._get_main_file",
            ...
            with mlflow.start_run() as run:
                tags = run.data.tags
                assert MLFLOW_GIT_BRANCH in tags
                assert tags[MLFLOW_GIT_BRANCH] == "feature/test"
                assert MLFLOW_GIT_REPO_URL in tags
...
def test_start_run_without_git_repo():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with mock.patch("mlflow.tracking.context.default_context._get_main_file",
            ...
            with mlflow.start_run() as run:
                tags = run.data.tags
                # Git tags should not be present
...
def test_manually_set_git_tags_not_overwritten():
    """Test that manually set Git tags are not overwritten"""
    manual_branch = "manual-branch"
    manual_repo = "https://manual.repo/url.git"
...
def test_nested_runs_inherit_git_metadata():
    """Test that nested runs inherit Git metadata from parent"""
    with tempfile.TemporaryDirectory() as tmpdir:
        repo = Repo.init(tmpdir)
        repo.create_remote("origin", "https://github.com/test/repo.git")
        ...
        with mock.patch("mlflow.tracking.context.default_context._get_main_file",
            ...
            with mlflow.start_run() as parent_run:
                ...
                with mlflow.start_run(nested=True) as child_run:
                    ...
                    # Child should have same Git metadata
...
**First, establish feature foundation by modifying the core GitRunContext:**
1. Import additional Git utility functions (`get_git_branch`, `get_git_repo_url`)
2. Import additional tag constants (`MLFLOW_GIT_BRANCH`, `MLFLOW_GIT_REPO_URL`)
3. Add cached properties for branch and repo URL retrieval
4. Extend `in_context()` to check for any Git metadata availability
5. Enhance `tags()` to include all available Git metadata

**Next, ensure quality by updating and creating comprehensive tests:**
1. Update existing test fixtures to mock branch and repo URL
2. Modify existing tests to verify all three Git tags
3. Add edge case tests for partial metadata availability
4. Add detached HEAD state test
5. Create integration test file for end-to-end validation

**Implementation follows minimal change principle:**
- Only modifies the specific methods needed in GitRunContext
- Leverages existing utility functions - no reimplementation
- Uses existing caching pattern - maintains consistency
- No changes to public APIs or interfaces
- No modifications to MLflow Projects components

#### Error Handling Strategy

```python
# Each Git utility function already handles errors gracefully:
...
#### GitRunContext will handle None values:
def tags(self):
    tags = {}
    if self._git_branch:  # Only add if not None
        tags[MLFLOW_GIT_BRANCH] = self._git_branch
...
```python
# Extend existing cache pattern
class GitRunContext(RunContextProvider):
    def __init__(self):
        self._cache = {}  # Single cache for all Git metadata
...
- Git operations performed only once per GitRunContext instance
- All three Git lookups share same repository discovery
- Total overhead: ~20-70ms on first access, 0ms on subsequent accesses
- No impact on non-Git environments (returns None immediately)
...
| File Pattern | Specific Changes |
|--------------|------------------|
| `mlflow/tracking/context/git_context.py` | Add branch/URL properties, extend tags() method |
| `tests/tracking/context/test_git_context.py` | Add assertions for new tags, edge case tests |
| `tests/tracking/fluent/test_fluent_git.py` | Create new integration test file |

**Files Used But NOT Modified:**

| File Pattern | Usage |
|--------------|-------|
| `mlflow/utils/git_utils.py` | Call existing `get_git_branch()` and `get_git_repo_url()` |
| `mlflow/utils/mlflow_tags.py` | Use existing `MLFLOW_GIT_BRANCH` and `MLFLOW_GIT_REPO_URL` constants |
| `mlflow/tracking/context/registry.py` | Existing registration of GitRunContext |
| `mlflow/tracking/context/abstract_context.py` | Inherit from RunContextProvider |
| `mlflow/tracking/context/default_context.py` | Use `_get_main_file()` function |
| `mlflow/tracking/fluent.py` | Existing `start_run()` uses context providers |
| `mlflow/tracking/client.py` | Existing `create_run()` uses context providers |
...
| `tests/tracking/fluent/test_fluent_git.py` | Integration tests for Git metadata |
| Mock Git repositories in tests | Temporary repos for testing |
| GitPython mock fixtures | Simulate Git operations |

**Git Scenarios In Scope:**

| Scenario | Expected Behavior |
|----------|-------------------|
| Standard Git repository | Capture branch and repo URL |
| Detached HEAD state | Capture commit and repo URL, no branch |
| No remote configured | Capture commit and branch, no repo URL |
| Multiple remotes | Use first remote (origin preferred) |
| Git submodules | Capture submodule's Git metadata |
| Nested directories | Find Git root via `search_parent_directories` |
| No Git repository | Return no Git tags |
| GitPython not installed | Log warning, return no Git tags |

#### Explicitly Out of Scope

**MLflow Projects Components (DO NOT MODIFY):**

| File/Pattern | Reason for Exclusion |
|--------------|---------------------|
| `mlflow/projects/utils.py` | Has separate manual Git tag implementation |
| `mlflow/projects/__init__.py` | Project execution logic unchanged |
| `mlflow/projects/submitted_run.py` | Run submission unchanged |
| `mlflow/projects/databricks.py` | Databricks project runner unchanged |
| `mlflow/projects/kubernetes.py` | Kubernetes project runner unchanged |
| `mlflow/projects/*.py` | All project-related files unchanged |

**MLflow Recipe/Pipeline Components:**

| File/Pattern | Reason for Exclusion |
|--------------|---------------------|
| `mlflow/recipes/*.py` | Recipe functionality unchanged |
| `mlflow/pipelines/*.py` | Pipeline functionality unchanged |

**Server and Storage Components:**

| File/Pattern | Reason for Exclusion |
|--------------|---------------------|
| `mlflow/server/*.py` | No server-side changes needed |
| `mlflow/store/db/*.py` | No database schema changes |
| `mlflow/store/artifact/*.py` | No artifact store changes |
| `mlflow/store/model_registry/*.py` | No model registry changes |

**API and Protocol Definitions:**

| File/Pattern | Reason for Exclusion |
|--------------|---------------------|
| `mlflow/protos/*.proto` | No protocol buffer changes |
| `mlflow/entities/*.py` | No entity model changes |
| REST API endpoints | No new endpoints needed |
| GraphQL schema | No GraphQL changes |

**Additional Exclusions:**

| Area | Reason for Exclusion |
|------|---------------------|
| UI Components | Git tags displayed using existing UI |
| Database migrations | Tags stored in existing tables |
| Authentication/Authorization | No security model changes |
| Model Registry | No impact on model versioning |
| Artifact handling | No changes to artifact storage |
| Autologging | Autologging uses standard APIs |
| Integrations (sklearn, pytorch, etc.) | Use standard tracking APIs |
...
| Validation | Method |
|------------|---------|
| Git metadata capture | Unit tests verify tags are set |
| Edge case handling | Tests for detached HEAD, no remotes |
| Non-interference with Projects | Verify Projects still sets tags manually |
| Performance impact | Ensure <100ms overhead |
| Thread safety | Context provider instance isolation |
| Backward compatibility | Existing code continues to work |

**What is NOT Being Validated:**

| Non-Validation | Reason |
|----------------|---------|
| Git tag accuracy | Trust GitPython library |
| Large repository performance | Standard Git operations |
| Network repository access | Local Git operations only |
| Git authentication | Not accessing remote servers |
| Concurrent Git operations | Git handles its own locking |

#### Feature Flag Considerations

**No feature flags required because:**
1. Enhancement is purely additive - no behavior removed
2. Graceful degradation when Git unavailable
3. Manual tags take precedence (not overwritten)
4. No breaking changes to any APIs
5. Minimal performance impact

**If feature flag were needed (NOT RECOMMENDED):**
```python
# Could use environment variable (but not implementing)
if os.environ.get("MLFLOW_DISABLE_AUTO_GIT_TRACKING") != "true":
    # Perform Git detection
```
...
| Concern | Mitigation |
|---------|------------|
| Repository URL exposure | Only captures configured remotes |
| Sensitive branch names | Uses existing tag security model |
| File system traversal | GitPython handles safely |
| Command injection | No shell commands executed |
| Information disclosure | Respects existing tag visibility |

**Security Non-Concerns:**

| Non-Issue | Reason |
|-----------|---------|
| Git credentials | Never captured or stored |
| SSH keys | Not accessed by implementation |
| Repository contents | Only metadata captured |
| Commit messages | Not included in tags |
| Author information | Not captured |

## 0.6 Special Instructions for Feature Addition

#### Feature-Specific Requirements

**Git Detection Pattern Requirements:**

1. **Use Existing GitPython Pattern**: The implementation MUST follow the defensive pattern already established in `mlflow/utils/git_utils.py`:
   ```python
   try:
       from git import Repo
   except ImportError:
       _logger.warning("GitPython is not installed")
       return None
   ```

2. **Search Parent Directories**: Always use `search_parent_directories=True` when creating Repo objects to handle nested directories:
   ```python
   repo = Repo(path, search_parent_directories=True)
   ```

3. **Handle File vs Directory Paths**: Check if path is file and use parent directory:
   ```python
...
|-------------|----------------|
| **Must not interfere with MLflow Projects** | Projects continues manual Git detection in `_create_run()` |
| **Must not override user tags** | User-provided tags in `start_run(tags={...})` take precedence |
| **Must work with nested runs** | Child runs inherit parent's Git context naturally |
| **Must support all tracking APIs** | Works via context provider registry |

**Performance and Scalability Considerations:**

1. **Caching is MANDATORY**: Git operations must be cached per GitRunContext instance:
   ```python
   @property
   def _git_branch(self):
       if "git_branch" not in self._cache:
           # Expensive operation only once
           self._cache["git_branch"] = get_git_branch(...)
       return self._cache.get("git_branch")
   ```

2. **Lazy Evaluation**: Git operations should only occur when tags are requested:
   ```python
   def tags(self):
       # Git operations happen here, not in __init__
   ```

3. **Fast Failure**: Return quickly when not in Git context:
   ```python
   if not main_file:
       return None  # Don't attempt Git operations
   ```
...
|-----------------|-------------|
| **Repository URL Sanitization** | Do not modify URLs - store exactly as configured |
| **Branch Name Validation** | Accept any valid Git branch name without filtering |
| **Error Messages** | Do not expose full paths in error messages |
| **Logging** | Use WARNING level for missing GitPython, no ERROR level |
...
|----------|------------------|
| **Windows** | GitPython handles path separators automatically |
| **Linux/macOS** | Standard POSIX paths work without modification |
| **Docker Containers** | Must handle cases where Git not installed |
| **Notebooks** | Detect main file from notebook kernel when possible |

**Edge Case Handling Matrix:**

| Edge Case | Required Behavior | Implementation |
|-----------|-------------------|----------------|
| **Detached HEAD** | No branch tag, commit and URL still captured | `get_git_branch()` returns None |
| **No remotes** | No repo URL tag, branch and commit still captured | `get_git_repo_url()` returns None |
| **Multiple remotes** | Use first remote, prefer "origin" if exists | Iterate `repo.remotes` |
| **Bare repository** | Return no tags | `Repo` constructor will fail gracefully |
| **Corrupt .git** | Return no tags | Exception caught and None returned |
| **Permission denied** | Return no tags | Exception caught and None returned |
| **Submodules** | Use submodule's Git info | GitPython handles correctly |
| **Worktrees** | Use worktree's branch | GitPython handles correctly |
...
|-----------|---------------------|
| **Happy path** | All three tags present |
| **No Git repo** | No tags added |
| **Partial metadata** | Only available tags added |
| **Detached HEAD** | Commit and URL, no branch |
| **No remotes** | Commit and branch, no URL |
| **GitPython missing** | Warning logged, no tags |
| **Manual tags** | Not overwritten |
| **Caching** | Git operations called once |

**Integration Test Requirements:**

| Test Scenario | Validation |
|---------------|------------|
| **Real Git repository** | Create temp repo, verify tags |
| **Fluent API** | `start_run()` captures tags |
| **Client API** | `MlflowClient.create_run()` captures tags |
| **Nested runs** | Child runs get Git context |
| **Projects compatibility** | Projects continues to work |
...
class GitRunContext(RunContextProvider):
    """
    Provides Git repository context for MLflow runs.
    
    Captures three pieces of Git metadata when available:
    - Git commit hash (mlflow.source.git.commit)
    - Git branch name (mlflow.source.git.branch)  # NEW
    - Git repository URL (mlflow.source.git.repoURL)  # NEW
    
    This context provider is automatically invoked when creating
    runs via mlflow.start_run() or MlflowClient.create_run().
    Git metadata is only captured when the code is executed
...
```python
def test_git_run_context_detached_head():
    """
    Test that GitRunContext correctly handles detached HEAD state.
    
    In detached HEAD, we expect:
    - Commit hash to be captured
    - Repository URL to be captured (if remotes exist)  
    - Branch name to be None (no active branch)
    """
```
...
1. Revert the single file change to `git_context.py`
2. No data cleanup required (tags are just metadata)
...
|--------|----------------|-----------------|
| Run creation time | <100ms increase | >500ms increase |
| Git tag presence | 60-80% of runs | <10% (GitPython issue) |
| Error rate | No increase | Any increase |
| Warning logs for GitPython | <5% of runs | >20% of runs |

**Log Messages to Monitor:**

```python
# Expected warning when GitPython not available
"GitPython is not installed. Git SHA will not be available."

#### Should NOT see errors like:
"Failed to capture Git metadata"  # We don't log errors
```
...
**✓ All user requirements mapped to specific implementation:**

| User Requirement | Implementation | File |
|------------------|----------------|------|
| Auto-detect Git branch | `get_git_branch()` called in GitRunContext | `git_context.py` |
| Auto-detect repo URL | `get_git_repo_url()` called in GitRunContext | `git_context.py` |
| No user code changes | Context provider auto-invoked | Via `registry.py` |
| Set system tags | `MLFLOW_GIT_BRANCH`, `MLFLOW_GIT_REPO_URL` | Via `tags()` method |
| Work with `start_run()` | Fluent API uses context registry | No change needed |
| Work with `MlflowClient` | Client uses context registry | No change needed |

**✓ Every integration point identified and documented:**

| Integration Point | Status | Notes |
|-------------------|--------|-------|
| Context provider registry | ✓ No change | Already registers GitRunContext |
| Fluent API | ✓ No change | Already calls resolve_tags |
| Client API | ✓ No change | Already uses registry |
| MLflow Projects | ✓ Protected | Separate implementation preserved |
| Tag storage | ✓ No change | Uses existing tag tables |

**✓ Test coverage planned for all new code:**

| Code Component | Test Coverage |
|----------------|---------------|
| Branch property | Unit test with mock |
| Repo URL property | Unit test with mock |
| Enhanced tags() | Unit test verifies all tags |
| Edge cases | Dedicated edge case tests |
| Integration | New integration test file |
| Caching | Test verifies single call |

**✓ Documentation covers usage and configuration:**

| Documentation | Status |
|---------------|--------|
| Class docstring | Updated in implementation |
| Method documentation | Properties documented |
| Test documentation | Test purposes documented |
| No user documentation needed | Transparent feature |

**✓ No orphaned files or unused code:**
- Only modifying existing GitRunContext class
- No new files except tests
- No deprecated code to remove

#### File Scope Validation

**✓ Every file in "In Scope" section has clear modification purpose:**

| File | Purpose | Validation |
|------|---------|------------|
| `git_context.py` | Add branch/URL capture | Core requirement |
| `test_git_context.py` | Test new functionality | Quality assurance |
| `test_fluent_git.py` | Integration testing | End-to-end validation |

**✓ Wildcard patterns capture all related files:**
- No wildcard patterns needed - specific files only
- All modifications isolated to tracking context module

**✓ No missing integration points:**

| Checked Area | Result |
|--------------|--------|
| Run creation paths | ✓ All use context registry |
| Tag precedence | ✓ User tags override |
| Error paths | ✓ Graceful None returns |
| Performance paths | ✓ Caching implemented |

**✓ Dependencies between files clearly mapped:**

```
git_context.py
├── imports from: git_utils.py (functions)
├── imports from: mlflow_tags.py (constants)
├── imports from: default_context.py (_get_main_file)
└── inherits from: abstract_context.py (RunContextProvider)

test_git_context.py
├── imports from: git_context.py (GitRunContext)
└── imports from: mlflow_tags.py (constants)

test_fluent_git.py
├── imports: mlflow (fluent API)
└── imports from: mlflow_tags.py (constants)
```
...
|------|------------|------------|
| Using existing functions | Low | Well-tested utilities |
| Following existing patterns | Low | Consistent with codebase |
| Adding to existing class | Low | Minimal changes |
...
| Item | Risk Level | Mitigation |
|------|------------|------------|
| Caching thread safety | Medium | Document known limitation |
| GitPython availability | Medium | Graceful degradation |
| Performance impact | Medium | Caching minimizes |

**High Risk Items ❌**

| Item | Risk Level | Status |
|------|------------|--------|
| Breaking Projects | Would be High | ✓ Avoided - no changes |
| API changes | Would be High | ✓ Avoided - no changes |
| Schema changes | Would be High | ✓ Avoided - no changes |

#### Pre-Implementation Checklist

**Developer should verify before starting:**

- [ ] GitPython is available in development environment
- [ ] Can run existing tests: `pytest tests/tracking/context/test_git_context.py`
- [ ] Understand existing GitRunContext implementation
- [ ] Review git_utils.py functions
- [ ] Understand context provider registry flow

#### Post-Implementation Checklist

**Developer should verify after implementation:**

- [ ] All existing tests still pass
- [ ] New tests pass
- [ ] No modifications to mlflow/projects/
- [ ] Manual test with real Git repository
- [ ] Manual test without Git repository
- [ ] Manual test with detached HEAD
- [ ] Performance overhead <100ms
- [ ] No errors in non-Git environments
...
1. **Functional Requirements Met:**
   - [x] Git branch automatically captured when available
   - [x] Git repo URL automatically captured when available
   - [x] Works transparently with no user code changes
   - [x] Does not interfere with MLflow Projects
...
3. **Documentation Complete:**
   - [x] Code documented
   - [x] Tests documented
   - [x] No user documentation needed (transparent)

#### Final Implementation Summary

**Total Scope: 3 files, ~200 lines of code**

| File | Lines Modified | Lines Added |
|------|----------------|-------------|
| `mlflow/tracking/context/git_context.py` | ~20 | ~30 |
| `tests/tracking/context/test_git_context.py` | ~10 | ~50 |
| `tests/tracking/fluent/test_fluent_git.py` | 0 | ~120 |
| **TOTAL** | ~30 | ~200 |

**This is a minimal, focused change that:**
- ✓ Adds functionality with minimal disruption
- ✓ Preserves all existing behavior
- ✓ Follows established patterns
- ✓ Includes comprehensive testing
- ✓ Requires no documentation changes
...
**Experiment Tracking**: Comprehensive logging and management of ML experiments through the fluent API in `mlflow/tracking/`, supporting parameter tracking, metrics collection, and artifact storage across multiple backend implementations. <span style="background-color: rgba(91, 57, 243, 0.2)">The tracking component now transparently captures additional Git metadata—commit hash, active branch name, and repository URL—as system tags for every run, requiring no user-side code changes.</span>
...
- **Plugin Architecture**: Extensible framework supporting custom storage backends and deployment targets
  - <span style="background-color: rgba(91, 57, 243, 0.2)">Run Context Providers: the GitRunContext plugin has been extended to cache and emit branch and repository URL information while preserving backward compatibility and avoiding interference with MLflow Projects or Recipes</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">Automatic Git context logging (commit, branch, repository URL) via enhanced GitRunContext; no user intervention required</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">Changes limited to tracking client modules; MLflow Projects and Recipe codepaths remain unmodified, ensuring independent Git tracking behavior is preserved</span>
...
**Description**
- **Overview:** Comprehensive logging and management of ML experiments through fluent APIs and client SDKs, supporting hierarchical experiment organization with advanced search and comparison capabilities. <span style="background-color: rgba(91, 57, 243, 0.2)">Automatically captures Git branch name (mlflow.source.git.branch) and repository URL (mlflow.source.git.repoURL) as system tags with zero code changes when runs are started inside a Git repository.</span>
...
- **User Benefits:** Track parameters, metrics, artifacts, and model versions across experiments with tag-based organization and metadata support. <span style="background-color: rgba(91, 57, 243, 0.2)">Transparent Git provenance tracking (branch & repository URL) to improve reproducibility and collaboration.</span>
- **Technical Context:** Supports multiple backend storage implementations (File/SQL/REST) with async logging queues for high-throughput scenarios. <span style="background-color: rgba(91, 57, 243, 0.2)">Enhanced GitRunContext provider leverages get_git_branch() and get_git_repo_url() utilities with caching and cross-platform support; respects manual tag precedence and maintains backward-compatible APIs.</span>
...
| <span style="background-color: rgba(91, 57, 243, 0.2)">F-001-RQ-005</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">Git Branch Tagging</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">When mlflow.start_run() (or MlflowClient.create_run()) is invoked inside a Git repository, the run MUST contain system tag `mlflow.source.git.branch` equal to the current branch name; must not be set when outside a Git repo; must not overwrite user-supplied tag with the same key</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">Must-Have</span> |
| <span style="background-color: rgba(91, 57, 243, 0.2)">F-001-RQ-006</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">Git Repo URL Tagging</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">When mlflow.start_run() (or MlflowClient.create_run()) is invoked inside a Git repository that has at least one remote, the run MUST contain system tag `mlflow.source.git.repoURL` equal to the first remote URL (origin preferred); must not be set when no remote exists; must not overwrite user-supplied tag with the same key</span> | <span style="background-color: rgba(91, 57, 243, 0.2)">Must-Have</span> |
...
- **Performance Criteria:** <100ms for parameter/metric logging, <500ms for search queries<span style="background-color: rgba(91, 57, 243, 0.2)">. Git metadata detection adds ≤100 ms to run creation (first run only due to caching).</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">**Git Integration Rules:** System tags beginning with `mlflow.source.git.*` are automatically populated only when detectable; manual values take precedence.</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">Integration Constraint: Only files under `mlflow/tracking/` and related utilities may be modified.</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Architectural Requirement: Changes must be isolated to run creation/initialization flow via GitRunContext provider.</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Interface Stability: Public APIs (`mlflow.start_run`, `mlflow.end_run`, `MlflowClient` methods) remain unchanged.</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Compatibility Constraint: Must not interfere with MLflow Projects or Recipe Git tracking; existing behaviour preserved.</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Edge-Case Handling: Implementation must gracefully handle detached HEAD, missing remotes, or absence of GitPython.</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Thread Safety & Caching: Git information retrieval must be cached per context instance to minimize overhead and avoid race conditions.</span>
...
| **<span style="background-color: rgba(91, 57, 243, 0.2)">Git Metadata Detection</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">≤100 ms added latency on first run, near-zero on subsequent runs via caching</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">Cached GitRunContext look-ups with lazy evaluation</span>** |
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">Repository URL is stored exactly as configured; no credentials or tokens are captured; manual tags, if provided, are never overwritten.</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">If GitPython is available and the execution path resides inside a Git repository, branch and repository URL will be automatically captured; otherwise, the feature degrades gracefully without affecting run creation</span>
...
- `2.1 Feature Catalog` - Git integration features and dependencies
- `2.4 Implementation Considerations` - Git integration constraints and performance requirements
...
- Git integration implementation analysis in `mlflow/tracking/` components
...
The experiment tracking process represents MLflow's foundational capability, orchestrating the complete lifecycle from run initialization through termination with comprehensive logging capabilities. <span style="background-color: rgba(91, 57, 243, 0.2)">The system automatically captures Git branch name (mlflow.source.git.branch) and repository URL (mlflow.source.git.repoURL) as system tags during run initialization when operating within a Git repository, requiring no code changes from users.</span>
...
flowchart TD
    ...
    D --> E
    E --> GIT1[Detect Git Metadata - branch, repo URL]
    GIT1 --> F[Set Environment Variables]
    ...
    subgraph "Git Integration"
        JJ["Skip if user already provided git tags"]
    end
    
    GIT1 -.-> JJ
```

<span style="background-color: rgba(91, 57, 243, 0.2)">Git metadata detection occurs transparently through the GitRunContext provider within the existing context provider registry. This process leverages caching mechanisms to minimize performance impact, typically adding less than 70ms overhead during run initialization. The detection process respects user-provided Git tags and maintains full backward compatibility with existing MLflow applications. When a Git repository is detected in the execution environment, the system automatically populates the mlflow.source.git.branch and mlflow.source.git.repoURL system tags, enhancing experiment reproducibility and collaboration workflows without requiring any modifications to user code.</span>
...
The tracking system serves as the foundation of MLflow's experiment management capabilities, providing comprehensive logging and organization of ML experiments. It manages experiment hierarchies, run lifecycles, parameter tracking, metrics collection, and artifact association with full support for concurrent operations and distributed environments. <span style="background-color: rgba(91, 57, 243, 0.2)">The system now automatically captures and stores Git branch and repository URL system tags for every run executed inside a Git repository, extending the existing commit hash detection to provide complete Git context tracking.</span>
...
<span style="background-color: rgba(91, 57, 243, 0.2)">**Git Integration**: GitPython library leveraged through mlflow.utils.git_utils.get_git_branch and get_git_repo_url for automatic Git metadata extraction during run creation within Git repositories.</span>

#### 5.2.1.3 Key Interfaces and APIs

**Fluent API**: High-level Python interface through `mlflow.start_run()`, `mlflow.log_param()`, `mlflow.log_metric()`, and `mlflow.log_artifact()` with automatic run context management. <span style="background-color: rgba(91, 57, 243, 0.2)">The mlflow.start_run() method automatically appends mlflow.source.git.branch and mlflow.source.git.repoURL system tags when executed within a Git repository, with user-supplied tags always taking precedence to maintain backward compatibility.</span>

**Client SDK**: Lower-level `MlflowClient` providing fine-grained control over experiment and run operations with explicit state management and error handling. <span style="background-color: rgba(91, 57, 243, 0.2)">The MlflowClient.create_run method transparently includes automatic Git metadata system tags alongside existing commit hash detection.</span>
...
<span style="background-color: rgba(91, 57, 243, 0.2)">**Git Metadata Caching**: In-memory cache extension within GitRunContext stores branch and repository URL alongside existing commit hash, ensuring less than 100 milliseconds additional overhead during run creation even under high-throughput scenarios with thousands of runs per minute.</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">**RunContext-level Git metadata cache**: Commit hash, branch name, and repository URL are cached per GitRunContext instance to eliminate redundant Git operations during tag resolution</span>
- **Artifact Cache**: Local filesystem caching for frequently accessed model artifacts

**Rationale**: Significantly improves system performance, reduces backend load, and provides better user experience for read-heavy workloads typical in ML experimentation. <span style="background-color: rgba(91, 57, 243, 0.2)">The Git metadata cache optimization specifically addresses the performance overhead of repeated Git repository introspection during high-frequency run creation scenarios.</span>

<span style="background-color: rgba(91, 57, 243, 0.2)">**Thread Safety Considerations**: The Git metadata cache remains instance-local within each GitRunContext rather than implementing global caching to avoid cross-thread mutation scenarios and preserve existing thread-safety guarantees during concurrent run creation. This design ensures that multiple threads creating runs simultaneously do not encounter race conditions or data corruption in Git metadata resolution.</span>
...
|---|---|---|---|
| **Tracking System Service** | Experiment management, run lifecycles, parameter tracking, metrics collection, <span style="background-color: rgba(91, 57, 243, 0.2)">Automatic Git metadata tagging (commit, branch, repository URL) as system tags</span> | Flask WSGI, SQLAlchemy ORM, AsyncLoggingQueue | Port 5000 (default), REST API |
...
<span style="background-color: rgba(91, 57, 243, 0.2)">The Tracking System Service automatically captures Git commit hash, branch name, and repository URL whenever `mlflow.start_run()` is invoked within a Git repository. This Git metadata is transparently tagged as `mlflow.source.git.commit`, `mlflow.source.git.branch`, and `mlflow.source.git.repoURL` system tags without requiring any user code changes. The functionality maintains full backward compatibility and provides seamless experiment provenance tracking for version-controlled machine learning projects.</span>
...
#### 6.3.4.5 Git Metadata Integration Flow (updated)

<span style="background-color: rgba(91, 57, 243, 0.2)">Starting with this release, GitRunContext automatically enriches run metadata with two new system tags — `mlflow.source.git.branch` and `mlflow.source.git.repoURL` — with no changes required to user code. This enhancement operates independently and does not interfere with MLflow Projects' existing manual Git tracking capabilities.</span>
...
**Git Metadata Security Controls**: <span style="background-color: rgba(91, 57, 243, 0.2)">Source control metadata handling with security-focused policies for reproducible experiment lineage and attack surface minimization.</span>

<span style="background-color: rgba(91, 57, 243, 0.2)">• *Repository URL Handling*: The system-tag `mlflow.source.git.repoURL` is persisted exactly as configured (no URL rewriting or sanitization) to guarantee reproducible experiment lineage.</span>

<span style="background-color: rgba(91, 57, 243, 0.2)">• *Branch Name Handling*: The system-tag `mlflow.source.git.branch` accepts any valid Git branch identifier without server-side filtering to avoid altering source-control semantics.</span>

<span style="background-color: rgba(91, 57, 243, 0.2)">• *Error Message Redaction*: Git-metadata collection failures never expose local filesystem paths. All such conditions emit WARNING-level logs only.</span>

<span style="background-color: rgba(91, 57, 243, 0.2)">• *Log Verbosity Policy*: Absence of GitPython or Git repository is logged at WARNING level, never ERROR, preventing false-positive security alerts.</span>
...
The `mlflow/system_metrics/` package provides comprehensive system monitoring capabilities with configurable sampling intervals and multi-node distributed training support. <span style="background-color: rgba(91, 57, 243, 0.2)">Enhanced with application-level monitoring for MLflow run lifecycle operations, including run creation performance tracking and Git metadata capture success monitoring.</span>
...
graph TB
    subgraph "System Metrics Architecture"
        ...
        A --> E[Network Monitor]
        A --> F[GPU Monitor]
        A --> G[Run Creation Monitor]
        
        subgraph "Collection Configuration"
            H[MLFLOW_ENABLE_SYSTEM_METRICS_LOGGING]
            I[MLFLOW_SYSTEM_METRICS_SAMPLING_INTERVAL]
            J[MLFLOW_SYSTEM_METRICS_SAMPLES_BEFORE_LOGGING]
            K[MLFLOW_SYSTEM_METRICS_NODE_ID]
        end
        
        B --> L[cpu_utilization_percentage]
        C --> M[system_memory_usage_megabytes]
        C --> N[system_memory_usage_percentage]
        D --> O[disk_usage_percentage]
        D --> P[disk_usage_megabytes]
        E --> Q[network_bytes_sent]
        E --> R[network_bytes_received]
        F --> S[gpu_memory_usage_percentage]
        F --> T[gpu_utilization_percentage]
        F --> U[gpu_power_usage_watts]
        G --> V[run_creation_duration_ms]
        G --> W[git_metadata_capture_success_ratio]
        
        subgraph "Metrics Export"
            L --> X[Prometheus Exporter]
            M --> X
            N --> X
            O --> X
            P --> X
        ...
        X --> Y[External Monitoring Systems]
    end
```
...
| **<span style="background-color: rgba(91, 57, 243, 0.2)">MLflow Run Lifecycle</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">`run_creation_duration_ms`</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">Histogram</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">60 seconds</span>** |
| **<span style="background-color: rgba(91, 57, 243, 0.2)">MLflow Run Lifecycle</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">`git_metadata_capture_success_ratio`</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">Gauge</span>** | **<span style="background-color: rgba(91, 57, 243, 0.2)">60 seconds</span>** |

#### 6.5.1.2 Log Aggregation System

#### Structured Logging Framework

MLflow implements JSON-structured logging throughout the system with consistent field naming conventions, correlation IDs for request tracking, and configurable log levels. The logging system is controlled via the `MLFLOW_CONFIGURE_LOGGING` environment variable (default: True). <span style="background-color: rgba(91, 57, 243, 0.2)">Enhanced with specialized log-based metrics for tracking GitPython dependency issues and their impact on Git metadata capture functionality.</span>
...
- **<span style="background-color: rgba(91, 57, 243, 0.2)">Dependency Monitoring</span>**: <span style="background-color: rgba(91, 57, 243, 0.2)">Log-based metric `gitpython_missing_warning_rate` tracking WARNING messages containing "GitPython is not installed", exported to Prometheus and included in log aggregation dashboards for proactive dependency management</span>
...
- **<span style="background-color: rgba(91, 57, 243, 0.2)">Performance Degradation (Warning)</span>**: <span style="background-color: rgba(91, 57, 243, 0.2)">Run creation time increase >500ms, indicating potential system performance issues requiring investigation</span>
- **<span style="background-color: rgba(91, 57, 243, 0.2)">Metadata Completeness (Critical)</span>**: <span style="background-color: rgba(91, 57, 243, 0.2)">Git tag presence <10%, indicating Git metadata capture degradation affecting experiment reproducibility</span>
...
- **<span style="background-color: rgba(91, 57, 243, 0.2)">Git Metadata Coverage: Percentage of MLflow runs that successfully record both mlflow.source.git.branch and mlflow.source.git.repoURL system tags (target range: 60-80% of runs), monitored through the git_metadata_capture_success_ratio gauge metric defined in section 6.5.1</span>**
...
|---|---|---|
| **Core Tracking** | `tests/tracking/`, `tests/entities/` | Experiment tracking, run management, metrics logging. <span style="background-color: rgba(91, 57, 243, 0.2)">Enhanced Git metadata unit tests located at `tests/tracking/context/test_git_context.py` covering automatic tagging of `mlflow.source.git.branch` and `mlflow.source.git.repoURL` as well as edge-case scenarios (detached HEAD, no remotes, caching).</span> |
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">**Git Repository Mocking**: Git repository mocking via patched GitPython `Repo` objects for branch, commit, and remote URL simulation</span>
...
- <span style="background-color: rgba(91, 57, 243, 0.2)">**Git Repository Fixtures**: Fixtures that generate temporary Git repositories for Git metadata tests</span>
...
**<span style="background-color: rgba(91, 57, 243, 0.2)">Git Metadata Fluent Integration</span>** (`tests/tracking/fluent/test_fluent_git.py`):
- <span style="background-color: rgba(91, 57, 243, 0.2)">Validation that `mlflow.start_run()` automatically attaches branch and repository URL tags</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Verification that manually supplied Git tags are respected and take precedence</span>
- <span style="background-color: rgba(91, 57, 243, 0.2)">Testing of nested run scenarios with proper Git metadata propagation</span>
...
<span style="background-color: rgba(91, 57, 243, 0.2)">Integration tests now verify tag-precedence behavior where user-provided Git tags remain unmodified, ensuring that manual Git tag specifications override automatic Git metadata detection.</span>
